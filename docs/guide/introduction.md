...
2. **Inline Small-Buffer Optimization (SBO)**: Shape and stride metadata for up to 8 dimensions are stored inline without allocating heap memory.
...
4. **Rich Scientific Stack**:
   ...
   - Sparse matrix representations (CSR/CSC) and iterative Krylov solvers (Conjugate Gradient, GMRES).
    - Pseudo-random number distributions (Uniform, Normal, integers, choice, shuffling, permutations).
...
pub fn main() !void {
    ...
    var eye = try num.identity(allocator, .{ .n = 2, .dtype = .f64 });
    defer eye.deinit();

    // Elementwise addition
    var sum_mat = try num.ops.add(a, eye, .{});
    defer sum_mat.deinit();

    // Matrix multiplication
    var prod = try num.linalg.matmul(a, eye, .{});
    defer prod.deinit();
}
```
